...
ለተጨማሪ ክለቦች MORE ብለው ይላኩ";
		  ...
		  sendSMS($msg_amh,$sender,8680);
		   ...
		   /// menue key like ok and more are checked her
			$sql = "SELECT * FROM `service_menu` WHERE `category_id` = 2 AND `key` = '$sms'";
			...
			if($keywords>0){
				...
				$messagesent=$row ['message_content'];
					sendSMS($messagesent,$sender,8680);
					...
					$sql = "SELECT * FROM `services` WHERE `category_id` = 2 AND `menu_key` = '$sms'";
					...
					if(mysql_num_rows(mysql_query($sql))>0)
					{
						 ...
						 $contact_id=$contact_id['id'];
							...
							$service_name_eng=$row ['name'];
									...
									if($FF>0)
									{
										...
										sendSMS("you have previously regisered for this sevice send OK or MORE to get new services.",$sender,8680);
										sendSMS(" ለዚህ አገልግሎት ቀደም ብለው ተመዝግበዋል ተጨማሪ አገልግሎት ከፈለጉ OK ወይም MORE ብለው ይላኩ።",$sender,8680);
									...
									$sql = "INSERT INTO `infocellvas`.`contact_group` (`id`, `contact_id`, `service_contact_id`,`category_id`, `start_date`, `end_date`, `status`, `created_at`, `update_at`)"
										."VALUES (NULL, '$contact_id', '$service_id','2', NULL, NULL, '1', CURRENT_TIMESTAMP, NULL);";
										mysql_query($sql);
									...
									//sendSMS("you have successfully subscribed to",$sender,8680);
									sendSMS("you have succssfully subscribed for ".$service_name_eng." Service",$sender,8680);
									//sendSMS($service_name_eng,$sender,8680);
									
									//sendSMS("temezgibewal",$sender,8680);
									sendSMS("ለ".$service_name_amh."  አገልግሎት ተመዝግበዋል",$sender,8680);
									//sendSMS($service_name_amh,$sender,8680);
						...
						$phonenumber=mysql_query($phonenumber);
						 ...
						 $contact_id=$contact_id['id'];
							...
							$sql = "INSERT INTO `infocellvas`.`contact_group` (`id`, `contact_id`, `service_contact_id`,`contact_id`, `start_date`, `end_date`, `status`, `created_at`, `update_at`)"
										."VALUES (NULL, '$contact_id', '$service_id','2', CURRENT_TIMESTAMP, NULL, '1', CURRENT_TIMESTAMP, NULL);";
										mysql_query($sql); 
							...
							if($sms=="stop")
							{
									...
									$sql = "SELECT * FROM `services` WHERE `category_id` = 2 AND `stop_key` = '$sms'";
									$sqlresult=mysql_query($sql);
								...
								die();
									...
									sendSMS("We didn't understand your comand.please send OK key code.",$sender,8680);
									sendSMS(" የላኩልንን መልዕክት መረዳት አልቻልንም እባኮትን OK ብለው እንደገና ይላኩ።",$sender,8680);
								}
