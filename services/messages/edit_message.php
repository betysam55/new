...
// print($_SESSION['is_user_has_client_message_permission']);
   $sql="UPDATE `draft_message` SET `content` = '$content' WHERE `draft_message`.`id` = $id;";
	$final=mysql_query($sql);
if ($final){
			$_SESSION['notification']='Contact created  successfully';
			header('Location: ../../messages/send_message.php?id='.$id.'&&category_id='.$category_id);
			
}

else{
	$_SESSION['session']='Failed to add Contact';
	header('Location: ../../messages/editmessage.php?id='.$id.'&&category_id='.$category_id);
